...
if [ -z $dir ]; then
    ...
    if [ -z $CHANGE_ID ]; then
        prNumber=$(gh pr list --state merged --json mergedAt,number,title | jq '.[]' | jq -s 'sort_by(.mergedAt) | reverse | first' | jq '.number')
    else
        prNumber=${CHANGE_ID}
    fi

    filesChanged=$(gh pr view $prNumber --json files --jq '.files.[].path')
