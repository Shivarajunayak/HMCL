...
terraform {
  source = "git@ssh.dev.azure.com:v3/hero-ev-team/CVPI%20-%20Thor%20Infra%20DevOps/thor-infra-terraform-modules//modules/pinpoint?ref=feature-sns"
}
...
inputs = {
  ...
  parameters = {
    ...
    hmcl-cv-pinpoint-app1 = {
      name = "hmcl-cv-${local.environment}-pinpoint-app1"
      tags = {
        Owner          = "dev-team"
        project-name   = "connected-vehicle-platform"
        Name           = "hmcl-cv-${local.environment}-pinpnt-app1"
        resource-type  = "pinpoint"
        resource-layer = "apps"
