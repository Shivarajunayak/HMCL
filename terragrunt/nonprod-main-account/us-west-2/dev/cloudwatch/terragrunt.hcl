...
terraform {
  source = "git@ssh.dev.azure.com:v3/hero-ev-team/CVPI%20-%20Thor%20Infra%20DevOps/thor-infra-terraform-modules//modules/cloudwatch"
}
...
inputs = {
  ...
  log_groups = [
    {
      name              = "hmcl-cv-${local.environment}-log-group-1"
      retention_in_days = 7
      ...
      tags = {
        ...
        Name           = "hmcl-cv-${local.environment}-cloudwatch"
    ...
    {
      name              = "hmcl-cv-${local.environment}-log-group-2"
  ...
  default_kms_key_id        = dependency.kms.outputs.key_arn
