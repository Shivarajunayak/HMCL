...
inputs = {
  ...
  cluster_name                   = "hmcl-cv-dev-esync-cluster"
  ...
  access_entries = {
    ...
    argocd_controller = {
      principal_arn = "arn:aws:iam::${local.aws_account_id}:role/hmcl-cv-${local.environment}-esync-cluster-${local.aws_region_short}-argocd-spoke-role"
      policy_associations = {
        policy = {
          policy_arn = "arn:aws:eks::aws:cluster-access-policy/AmazonEKSClusterAdminPolicy"
          access_scope = {
  ...
  tags = {
    ...
    Name                = "hmcl-cv-${local.environment}-esync-eks"
