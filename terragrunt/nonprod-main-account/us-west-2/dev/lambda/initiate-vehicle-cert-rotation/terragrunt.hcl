...
inputs = {
  ...
  function_name          = "hmcl-cv-${local.environment}-initiate-vehicle-cert-rotation"
  description            = "Vehicle cert Lambda"
  ...
  s3_existing_package = {
    ...
    key    = "initiate-vehicle-cert-rotation-api/hmcl-cvp-initiate-vehicle-cert-rotation.zip"
  }
  ...
  environment_variables = {
    ...
    IOT_POLICY_NAME                              = "hmcl-cv-${local.environment}-vehicle-cert-rotation-iot-policy"
    VALIDITY                                     = 5
    VALIDITY_TYPE                                = "YEARS"
    REGION                                       = "${local.aws_region}"
    TOPIC                                        = "hmcl/tcu/{tcuId}/rotation/crt"
  }
  ...
  role_name                         = "hmcl-cv-${local.environment}-${local.aws_region_short}-initiate-vehicle-cert-rotation-role"
